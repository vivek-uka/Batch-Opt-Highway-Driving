...
ax.set_xticklabels(["Ours", "ACADO_11", "ACADO_6", "Frenet"], fontsize=10, weight='bold')
...
plt.subplot(2,3,2)
sns.set_theme(style="whitegrid")
ax = sns.boxplot(data = [ours[:,3], acado[:,3], acado_2[:,3], frenet[:,3]], linewidth = 2, showfliers=False)
ax.set_xticklabels(["Ours", "ACADO_11", "ACADO_6", "Frenet"], fontsize=10, weight='bold')
...
plt.subplot(2,3,5)
sns.set_theme(style="whitegrid")
ax = sns.boxplot(data = [where(ours[:,1] > -8, abs(ours[:,1] - (-8)), 0),where(acado[:,1] > -8, abs(acado[:,1] - (-8)), 0), where(acado_2[:,1] > -8, abs(acado_2[:,1] - (-8)), 0), where(frenet[:,1] > -8, abs(frenet[:,1] - (-8)), 0)], linewidth = 2, showfliers=False)
ax.set_xticklabels(["Ours", "ACADO_11", "ACADO_6", "Frenet"], fontsize=10, weight='bold')
